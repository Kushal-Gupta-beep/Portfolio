...
const fallbackProjects = [
  ...
  },
  {
    ...
    description: 'A voice-driven shopping assistant that enables users to search, compare, and purchase products using natural language conversations — powered by speech-to-text, NLP pipelines, and an AI recommendation engine.',
    highlights: [
  ...
  },
  {
    ...
    name: 'CommitIQ',
    category: 'AI / NLP',
    description: 'An intelligent commit analysis system that evaluates GitHub commit histories using NLP pipelines — classifying commits by purpose and impact, generating repository-level metrics, and automated scoring.',
    highlights: [
  ...
  },
  {
    ...
    description: 'A Web3 gifting platform enabling secure ETH transfers with personalized messages — leveraging blockchain for transparent, global transactions.',
    highlights: [
...
async function handlePost(req: VercelRequest, res: VercelResponse) {
  ...
  const { number, name, category, description, highlights, tags, stats, gradient, repoUrl, liveUrl } = req.body
  const { data, error } = await supabase
    .from('projects')
    .insert({ number, name, category, description, highlights, tags, stats, gradient, repoUrl, liveUrl })
    .select()
    .single()
